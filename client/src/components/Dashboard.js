...
const Dashboard = () => {
    ...
    const handleCreatePage = async () => {
        const token = localStorage.getItem('token');
        try {
            const response = await axios.post('http://localhost:5000/api/pages', { title, description, links }, {
                headers: { Authorization: `Bearer ${token}` }
            });
        ...
        <div>
            ...
            {showPopup && (
                <div className="popup">
                    ...
                    <button type="button" onClick={editMode ? handleEditPage : handleCreatePage}>
                        ...
                        <div key={page._id} className="page-card">
                            ...
                            <Link to={`/pages/${page._id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                                <div>
                                    ...
                                    {/* Display view count */}
                                    <p><strong>Views:</strong> {page.viewCount}</p> {/* Assuming viewCount is part of your page model */}
                                </div>
                            </Link>
